...
- エラー通知（トースト）は、レンダー由来の状態を Effect で監視して出すのではなく、失敗が起きるコールバックから発火する。データ取得の失敗は SWR の `onError` に集約する
  ...
  - 補足: そもそも通常環境では起きない防御的な失敗（例: `Intl` が壊れて今日の JST 日付を解決できない）に、通知経路を用意して手厚く扱わない。握りつぶさず素直に投げ、ルート直下のエラーバウンダリに委ねる
