...
- `as unknown as T`（二重アサーション）は原則使わない。まず単一の `as T` で通らないか試し、通るなら単一にする
  ...
  - 例外: 判別共用体の解決値など、単一 `as` では型が重ならず二重が避けられない場合のみ使い、その旨を disable コメントの理由に書く（例: `notification/src/discord.test.ts` の `createMockLogger`。private フィールドを持つ型へ構造的に代入できず二重が避けられない）。まずは完全な型を組み立てる／throw で再現する等でアサーション自体を無くせないかを優先検討する
