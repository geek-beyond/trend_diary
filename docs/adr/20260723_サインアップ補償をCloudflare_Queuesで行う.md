...
# 追記: 定期掃除バックストップの判定基準（#1041）

本 ADR で対象外とした案D（定期掃除）を、キュー補償が構造的に救えない失敗（signUp 成功〜キュー送出間の Worker クラッシュ、保持 24 時間超のコンシューマ障害）のバックストップとして `apps/cron` に設ける。削除対象の判定基準を以下のとおり決定する。

- **対象経路**: identities が `email` のみ（パスワードサインアップ由来）の認証ユーザーに限定する
  - OAuth 経路は次回ログインで自己修復される上、削除するとプロバイダ連携ごと失われるため対象外とする（本文 Context の判断を踏襲）
  - passkey は既存ユーザーへの追加ログイン手段であり、新規の認証ユーザーを作らないため考慮不要
- **孤児の判定**: 認証ユーザーの id に紐づく行が `active_users`（`authentication_id`）に存在しないこと
- **猶予期間**: 認証ユーザー作成から 24 時間。正規の登録は signUp と同一リクエスト内の数秒で完了するため誤削除リスクは実質なく、キュー補償（保持 24 時間・リトライ含む）の決着後にのみ掃除することで補償との競合も避ける
- **件数上限**: 1 回の実行あたり 10 件。超過分は次回実行に持ち越す。判定バグによる大量削除の爆発半径を抑えるためのガードであり、孤児は登録失敗時にしか生まれず稀のため、上限到達自体は異常（Discord 通知の対象）としない
- **走査方法**: `auth.admin.listUsers` はページネーションで全件走査する。現規模では 1 ページに収まるが、件数増で静かに掃除漏れする形を避ける
- **実行頻度**: 日次。即時性はキュー補償が担うため、バックストップは低頻度で足りる
