...
2. 本番: CD（`cd.yaml`）が `wrangler d1 migrations apply trend-diary-db --remote` を自動実行します
   （`cloudflare/wrangler-action`、working-directory は `application/apps/web`）。pnpm script としては提供していません。
...
ローカルでも `application/` ディレクトリで上記をそのまま実行すれば同じ検証ができます
（`git` のパス指定は CI の `working-directory: ./application` と同じく `application/` 基準です。
