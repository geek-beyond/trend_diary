...
| --- | --- | --- | --- |
| `git commit` | [`pre-commit.sh`](../../scripts/git-hooks/pre-commit.sh) | lint（typecheck 含む） | code-quality |
| `git push` | [`pre-push.sh`](../../scripts/git-hooks/pre-push.sh) | packages の単体テスト | unit-test |
...
# 再度有効化
git config --local hook.pre-commit-lint.enabled true
git config --local hook.pre-push-test.enabled true
```

## 仕組み（参考）

git 2.54 から、フックを設定ファイル（`hook.<name>.event` / `hook.<name>.command`）で宣言できるようになりました。本リポジトリの定義は [`.gitconfig-hooks`](../../.gitconfig-hooks) にあります。

- 同一イベントに複数フックを割り当てられ、設定の出現順に逐次実行されます
