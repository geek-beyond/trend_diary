...
| 環境 | 設定箇所 | 値 |
| --- | --- | --- |
| 本番 web | `application/apps/web/wrangler.toml` の `[vars]` | `LOG_LEVEL = "info"` |
| 本番 cron | `application/apps/cron/wrangler.toml` の `[vars]` | `LOG_LEVEL = "info"` |
| ローカル | `application/apps/web/.dev.vars`（`.dev.vars.example` 参照） | 未設定なら `info` |
...
- **アプリの 5xx / 未捕捉エラー**: `application/apps/web/src/middleware/error-handler.ts` が `DiscordNotifier`（`application/packages/notification/src/discord.ts`）でエラーメッセージ・リクエスト情報・スタックトレースを embed 送信する。
- **cron ジョブのエラー**: `application/apps/cron/src/worker.ts` が `DiscordWebhookClient` で通知する。
...
- **ローカル**: `application/apps/web/.dev.vars` に記載する（`.dev.vars.example` を参照。`.dev.vars` は Git 管理外）。
