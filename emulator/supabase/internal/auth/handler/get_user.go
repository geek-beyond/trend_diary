...
func (h *GetUser) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if token == "" {
		...
		resp.APIErrorWithCode(http.StatusUnauthorized, "no_authorization",
	...
	if err != nil {
		...
		resp.APIErrorWithCode(http.StatusUnauthorized, "bad_jwt", "invalid JWT: "+err.Error())
		return
	}
	u, ok := h.store.FindUserByID(claims.Subject)
	if !ok {
		// 署名は通ったが該当 user が消えている状態。これだけが本当の session_not_found。
		resp.APIErrorWithCode(http.StatusUnauthorized, "session_not_found",
			"AuthSessionMissingError: Auth session missing!")
		return
	}
	resp.JSON(http.StatusOK, u)
}
