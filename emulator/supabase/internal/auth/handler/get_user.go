...
func GetUser(h *Handler) {
	...
	if token == "" {
		...
		h.ErrorCode(http.StatusUnauthorized, "no_authorization",
	...
	if err != nil {
		...
		h.ErrorCode(http.StatusUnauthorized, "bad_jwt", "invalid JWT: "+err.Error())
		return
	}
	u, ok := h.store.FindUserByID(claims.Subject)
	if !ok {
		// 署名は通ったが該当 user が消えている状態だけが本当の session_not_found。
		h.ErrorCode(http.StatusUnauthorized, "session_not_found",
